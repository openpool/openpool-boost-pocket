...
Sep 18, 2013 power saving function added by @kakenman.
Oct  2, 2013 revised for Unity version
To use this version, you have to install custom firmware described below.
...
// Need PowerControl files at this URL
// http://mbed.org/users/no2chem/notebook/mbed-power-controlconsumption/


#define BAUDRATE 115200
#define VERSION_STR "Pocket_detector#1.00\n"
    ...
    while(1) {                           //wait for start command
        if(pc.readable()) {
            ...
            if(buff == 's') {
                ...
                pc.printf(VERSION_STR);
            } else if(buff == 'd') {
                debug_mode = true;
                pc.printf("entering debug mode\n");
            } else if(buff == 'b') {
                debug_mode = false;
                pc.printf("finishing debug mode\n");
            }
        }
    }
    fallTimer.attach_us(&setFallSenseFlag,100);  //sensing timer start

    // ball index for debug mode
    int ball_emu_index = 0;
        ...
        if(pc.readable()) {               //if serial buffer is readable
            ...
            if(buff == 'r') {        //check command
                if(debug_mode) {
                    ...
                    ball_emu_index = (ball_emu_index + 1 ) % 6;
                    fallCount[ball_emu_index] = 1;
                }
